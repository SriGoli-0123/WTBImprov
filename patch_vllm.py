...
def main():
    ...
    if match:
        ...
        #    brackets, then retry - so the call is preserved instead of dropped;
        # 3) only warn when even the repaired string is unusable.
        replacement = (
            ...
            + indent + "                fixed += \"}\" if opener == \"{\" else \"]\"\n"
            + indent + "            try:\n"
            + indent + "                obj = json.loads(fixed)\n"
            + indent + "                if isinstance(obj, dict) and \"name\" in obj:\n"
            + indent + "                    raw_function_calls.append(obj)\n"
            + indent + "                    parsed_any = True\n"
